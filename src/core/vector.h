...
template<typename T>
struct alignas(2 * sizeof(T)) vector_storage<T, 2> {
    COMPAS_HOST_DEVICE
...
template<typename T>
struct alignas(2 * sizeof(T)) vector_storage<T, 3> {
    COMPAS_HOST_DEVICE
...
template<typename T>
struct alignas(4 * sizeof(T)) vector_storage<T, 4> {
    COMPAS_HOST_DEVICE
...
struct vector: vector_storage<T, N> {
    ...
    COMPAS_HOST_DEVICE
    vector(const T& first, const T& second, const Ts&... rest) :
        storage_type(first, second, rest...) {}

    COMPAS_HOST_DEVICE
    T& at(index_t i) {
        COMPAS_DEBUG_ASSERT(i >= N && i < N);
        return this->data()[i];
    }
...
template<typename T>
COMPAS_HOST_DEVICE vector<T, 3>
cross(const vector<T, 3>& left, const vector<T, 3>& right) {
    return {
...
operator==(const vector<T, N>& left, const vector<R, M>& right) {
    if (N != M) {
        return false;
    }
