...
function __init__()
  ...
  if expected_version != gotten_version
    throw(ErrorException("version of `$LIBRARY` is invalid: got version $gotten_version, expected $expected_version"))
  end
end
...
function assert_size(input, expected::Dims{N}) where N
 gotten = size(input)
  if gotten != expected
...
function convert_array(ty::Type{T}, dims::Dims{N}, input::Array{T, N})::Array{T, N} where {T, N}
  assert_size(input, dims)
  return input
end

function convert_array(ty::Type{T}, dims::Dims{N}, input::AbstractArray)::Array{T, N} where {T, N}
  assert_size(input, dims)
